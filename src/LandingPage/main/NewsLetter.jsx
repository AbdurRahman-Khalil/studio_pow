...
const NewsLetter = ({ newsLetterHeading, newsLetterPara, placeholderText }) => {
    return (
        <section id='newsletter' className="text-slate-200 h-[fit-content] w-full mt-20 mb-8">
            <div className='flex flex-col items-center text-center w-[100%] h-[100%] bg-black/15 backdrop-blur-2xl py-14 px-5 min-[300px]:px-6 sm:px-12 md:px-18 lg:px-20'>
                <h2 className='mb-4 text-3xl font-bold font-montserrat'>{newsLetterHeading}</h2>
                <p className='pr-1 mb-6 text-lg font-normal tracking-wide sm:font-light'>{newsLetterPara}</p>
                <form className='flex gap-3 px-3 py-3 rounded-md bg-white/50'>
                    <input
                        type='email'
                        id='email'
